...
from PIL import Image # NECESSÁRIO PARA CORRIGIR O ERRO
...
# --- FUNÇÃO PARA CRIAR IMAGEM PIL ---
def get_pitch_pil_image(df_to_plot):
    ...
    # Definimos um tamanho fixo para facilitar o mapeamento de coordenadas
    ...
    plt.close(fig)
    buf.seek(0)
    return Image.open(buf) # RETORNA OBJECTO PIL IMAGE
...
# MOSTRAR CAMPO E CAPTURAR CLIQUE
st.subheader("📍 Clica no Campo para Marcar")
if v_mode == "Seta":
    if st.session_state.temp_coords is None:
        st.info("Clica no **INÍCIO** da ação.")
    else:
        st.warning("Clica no **FIM** da ação.")

pil_img = get_pitch_pil_image(df_view)
# Forçamos uma largura para o mapeamento ser consistente
value = streamlit_image_coordinates(pil_img, width=800, key="pitch_click")

if value:
    # Mapeamento: 800px largura = 105m | 518px altura = 68m
    x_m = (value['x'] / 800) * 105
...
if not st.session_state.actions.empty:
    st.markdown("---")
    def generate_pdf(df_filt, fig_p, title):
        pdf = FPDF()
        pdf.add_page()
        ...
        pdf.set_y(175); pdf.set_font("Helvetica", "B", 10)
        pdf.cell(190, 10, "Tabela de Resultados", ln=True)
        # Cabeçalho da Tabela
        pdf.set_fill_color(230,230,230)
        h = ["Acao", "Sucesso", "Insucesso", "Total", "xG"]
        w = [40, 35, 35, 35, 35]
        for i, text in enumerate(h): pdf.cell(w[i], 7, text, border=1, fill=True, align="C")
        pdf.ln()
        pdf.set_font("Helvetica", "", 9)
        for act in df_filt['Ação'].unique():
            t = df_filt[df_filt['Ação'] == act]
            s = len(t[t['Resultado'] == 'Sucesso'])
            f = len(t[t['Resultado'] == 'Insucesso'])
            pdf.cell(w[0], 7, act, border=1)
            pdf.cell(w[1], 7, str(s), border=1)
            pdf.cell(w[2], 7, str(f), border=1)
            pdf.cell(w[3], 7, str(s+f), border=1)
            pdf.cell(w[4], 7, f"{t['xG'].sum():.2f}", border=1)
            pdf.ln()
        return bytes(pdf.output())

    c1, c2 = st.columns(2)
    with c1:
        # Gerar imagem estática final para o PDF
        ...
        for _, row in df_view.iterrows():
            if row['Visualizacao'] == "Seta": p_final.arrows(row.x, row.y, row.end_x, row.end_y, width=2, color=row.Cor, ax=ax_f)
            else: p_final.scatter(row.x, row.y, s=180, c=row.Cor, marker='o' if row['Resultado']=='Sucesso' else 'X', ax=ax_f)
        
        pdf_b = generate_pdf(df_view, fig_f, report_title)
        st.download_button("📥 Descarregar PDF", pdf_b, "relatorio_fmh.pdf", "application/pdf")
