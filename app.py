...
if os.path.exists(fmh_logo_path):
    st.sidebar.image(fmh_logo_path, width=100)
...
report_title = st.sidebar.text_input("Título do Relatório PDF", "Relatório Técnico-Tático")
...
def get_pitch_pil(df_plot):
    ...
    for _, row in df_plot.iterrows():
        if row['Visualizacao'] == "Seta":
            # Setas melhoradas: a ponta termina exatamente na coordenada
...
if not st.session_state.actions.empty:
    ...
    with col_m:
        ...
        idx_del = st.selectbox("Apagar por ID:", st.session_state.actions.index, format_func=lambda i: f"ID {i}: {st.session_state.actions.loc[i, 'Jogador']} - {st.session_state.actions.loc[i, 'Ação']}")
    ...
    def generate_pdf(df_filt, fig_p, title_r, current_sel_a):
        ...
        has_rem = "Remate" in df_filt['Ação'].values
        h = ["Acao", "Sucesso", "Insucesso", "Total", "xG Acum."] if has_rem else ["Acao", "Sucesso", "Insucesso", "Total"]
        ws = [45, 35, 35, 35, 40] if has_rem else [55, 45, 45, 45]
        ...
        for act in df_filt['Ação'].unique():
            ...
            if has_rem: pdf.cell(ws[4], 8, f"{temp['xG'].sum():.2f}" if act == "Remate" else "-", border=1, align="C")
            pdf.ln()
        ...
        if has_rem: pdf.cell(ws[4], 8, f"{x_t:.2f}", border=1, fill=True, align="C")
        pdf.ln(10)
        if (s_t + i_t) > 0:
            perc = (s_t / (s_t + i_t)) * 100
